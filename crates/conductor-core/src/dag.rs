...
///
/// Checks for:
/// 1. Invalid dependency references (out of bounds, self-dependency)
...
pub fn validate_plan(tasks: &[Task]) -> PlanValidation {
    ...
    for cycle in &cycles {
        let names: Vec<String> = cycle
            .iter()
            .map(|&i| {
                let title = tasks.get(i).map(|t| t.title.as_str()).unwrap_or("?");
                format!("{i} (\"{title}\")")
            })
            .collect();
        issues.push(ValidationIssue {
    ...
    for i in 0..tasks.len() {
        for j in (i + 1)..tasks.len() {
            let overlap: Vec<&String> = tasks[i]
                ...
                .collect();
            if !overlap.is_empty() {
                let has_dep_link = tasks[i].dependencies.contains(&j)
                    || tasks[j].dependencies.contains(&i);
                if !has_dep_link {
                    ...
                    issues.push(ValidationIssue {
                        ...
                        message: format!(
                            ...
                            i, tasks[i].title, j, tasks[j].title, files.join(", ")
                        ),
                        task_indices: Some(vec![i, j]),
                    });
                }
            }
        }
