...
pub fn render_prompt_bar(
    ...
    let input_spans = if input_text.starts_with('/') {
        ...
        let cmd_color = if is_valid { THEME.warning } else { THEME.text_muted };
        ...
        vec![Span::styled(input_text, Style::default().fg(THEME.text_primary))]
    };
    ...
    let visible_spans = if input_text.starts_with('/') && h_scroll == 0 {
        ...
        vec![Span::styled(&visible_input, Style::default().fg(THEME.text_primary))]
    };
    ...
    let prefix_spans: Vec<Span<'static>> = if phase_label.is_empty() {
        ...
        vec![
            Span::raw(" "),
            Span::styled(phase_label.to_string(), Style::default().fg(THEME.text_muted)),
            Span::raw(" "),
            Span::styled("›", Style::default().fg(THEME.accent)),
            Span::raw(" "),
        ]
    ...
    let prompt = Paragraph::new(Line::from(spans)).style(Style::default().bg(THEME.surface));
...
pub fn render_keyboard_help(f: &mut Frame, area: Rect) {
    ...
    let inner = render_borderless_panel(f, popup, Some("Keyboard Shortcuts"), THEME.surface);
    ...
    let rows = vec![
        ...
        Row::new(["", ""]),
        Row::new(["  COMMANDS", ""]).style(Style::default().fg(THEME.accent)),
